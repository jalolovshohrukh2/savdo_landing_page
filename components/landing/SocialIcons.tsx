/**
 * Brand glyphs for the footer social links. Lucide dropped its brand icons,
 * so these are inline SVGs (fill: currentColor) that inherit the link color
 * and theme cleanly — no CDN, no empty masks.
 */
type IconProps = { className?: string };

const base = (className: string) => `${className}`;

export function TelegramIcon({ className = 'h-[18px] w-[18px]' }: IconProps) {
  return (
...
export function FacebookIcon({ className = 'h-[18px] w-[18px]' }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden className={base(className)}>
